...
async function fetchAndStoreJobs() {
    try {
        ...
        const [remotiveResponse, arbeitnowResponse, remoteokResponse] =
            await Promise.all([remotivePromise, arbeitnowPromise, remoteokPromise]);
        ...
        const remoteokJobs = remoteokResponse.data
        ...
        const combined = [...remotiveJobs, ...arbeitnowJobs, ...remoteokJobs];
...
app.get("/api/jobs", async (req, res) => {
    try {
        ...
        const perSource = Math.floor(limit / 3);
        ...
        let combined = [...remotive.rows, ...arbeitnow.rows, ...remoteok.rows];
...
app.get("/api/reset", async (req, res) => {
    try {
        ...
        res.json({ message: "Jobs table cleared." });
    } catch (error) {
        res.status(500).json({ error: "Failed to reset jobs table." });
    }
});
